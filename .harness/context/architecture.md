...
- `infrastructure/memory/InMemoryRoomRepository` conserva el agregado sala y su indice secundario `playerId -> roomCode` en `Map`: cada `save` reindexa atomically la copia reemplazada, y `delete`/`clear` limpian solo sus asociaciones vigentes. No existe `PlayerRepository`; `SessionRegistry` sigue siendo transporte.
- `infrastructure/runtime` implementa `SystemClock` y `SystemRandomSource` como propietarios de `Date.now` y `Math.random`; `ManualClock` y `SequenceRandomSource` hacen reproducibles instante, avance, orden de azar y lecturas en pruebas. El composition root crea una instancia compartida de reloj y azar por runtime y la inyecta en aplicación, presenter, publisher y resync para que un envelope use el mismo instante en tiempo y capacidades.
- `infrastructure/scheduling/` implementa el puerto `Scheduler`: `ProcessScheduler` posee la espera `setTimeout` y la entrega `setImmediate` de cada identidad `{ roomCode, trigger }`, conserva generación y retira una entrada activa exactamente una vez antes de `run`; `DeterministicScheduler` usa el reloj inyectado y expone `pendingJobs`, orden estable por vencimiento/secuencia e historial para pruebas sin espera real.
...
`ApplicationResult` expresa todo trabajo diferido como directivas `replace`, `cancel` o `cancel-room`. Tras guardar el agregado, `dispatchApplicationResult` materializa primero cancelaciones, después publica los eventos ya comprometidos en su orden wire y por último cancela/reemplaza la clave de cada `replace`. El puerto también ofrece `cancelAll` para lifecycle. La matriz de ownership/cleanup es: `replace` cancela la identidad anterior y registra su reemplazo; expiry retira su entrada antes de reinyectar el efecto; retry y borrado de última persona declaran `cancel-room`; el consenso de estrella declara `cancel` de `cpu-turn`; `resetServerForTests` y `stopServer` invocan `cancelAll` antes de limpiar repositorio, sesiones o Socket.IO. Las guardas de versión, fase, lock y deadline de `EffectUseCases` conservan la defensa final para callbacks tardíos.

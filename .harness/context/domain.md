...
| Desconocido | Iniciar manualmente por host con dos conectados; usar ready cooperativo solo para reanudar la ronda desde `focus` o `paused`, excluyendo conectados sin cartas del quorum. | Implementado | `apps/backend/src/domain/setup.ts`, `apps/backend/src/domain/round.ts`, `apps/backend/src/domain/stateMachine.ts` |
...
| Desconocido | Resolver una estrella solo por consenso y descartar la carta mas baja de cada mano no vacia tras el cierre visual. | Implementado | `apps/backend/src/domain/star.ts`, `application/starUseCases.ts`, `effectUseCases.ts` y `registerGameHandlers.ts` |
...
- Estrella cooperativa: `domain/star.ts` posee propuesta, votos, consenso, consumo unico, preview y settlement idempotente; `application/starUseCases.ts` conserva la espera visual por identidades humanas y `effectUseCases.ts` recibe una sola transicion `star-settled` desde ack, desconexion o deadline. Evidencia adicional en `transport/socket/registerGameHandlers.ts` y `apps/frontend/src/starUi.ts`.
...
# Maquina de estados de partida
 ...
 La fuente canónica de autorización es `apps/backend/src/domain/stateMachine.ts`. Las fases son públicas; locks y settlement de estrella son ejes ortogonales y no fases adicionales. La UI presenta fases, locks y animaciones, pero solo las capacidades privadas autorizan comandos.
...
- `domain/stateMachine.ts` sigue siendo la autoridad única de fase, actor, lock, expiración y terminales. `domain/room.ts` posee admisión/reconexión/expulsión; `application/roomUseCases.ts` orquesta sala y desconexión por identidad sin sockets. `application/gameUseCases.ts` posee inicio, retry, ready, pausa, jugada y CPU. `application/starUseCases.ts` posee propuesta, voto, cancelación, rechazo y acknowledgement visual, y conserva el efecto de settlement más los humanos pendientes. Solo la pausa manual declara `game:paused` y su log; las pausas automáticas siguen silenciosas. `application/effectUseCases.ts` reinyecta `dealing-expired`, `countdown-expired`, `error-expired`, `round-flip-expired`, `round-unflip-expired`, `next-level-expired`, `level-ready-expired`, `cpu-turn` y `star-settled` con su sala, versión, fase, lock y deadline originales; una sala borrada, versión distinta, retry o lock reemplazado no guarda ni publica.
...
Invariantes confirmadas de setup y ronda: el balance soportado es 2--8 jugadores (`maxLevel`/vidas: 2=12/2, 3=10/3, 4--5=8/4, 6=7/5, 7=6/5, 8=5/5); el inicio y retry crean nivel 1, una estrella, mapa de recompensas truncado al nivel maximo y manos ordenadas tomadas secuencialmente del mazo inyectado. Inicio conserva el ready humano existente y marca ready a CPU; retry y reparto de nivel reinician ready humano a `false` y marcan ready a CPU. `GameUseCases.setPlayerReady` y `requestPause` usan el reloj inyectado y no mutan su entrada: el dominio vuelve a conectar y marcar ready a CPU, y una pausa borra ready solo de conectados con cartas. Una mano vacía conectada no integra el quorum ni puede hacer ready/pausa; una pausa manual publica primero snapshot, después `game:paused` y su log. Un quorum completo declara countdown con `dueAt`, fase, razón y deadline esperados; `EffectUseCases` lo materializa con la copia original y su expiración solo opera si versión, fase, lock y deadline aún coinciden y el deadline ya venció.

Invariantes confirmadas de cartas: una jugada valida contiene la carta minima propia y se registra una sola vez en pila e historial con `now` inyectado. Las bloqueantes son todas las cartas menores aun en cualquier mano, ordenadas por valor e identidad; una penalizacion pierde exactamente una vida sin bajar de cero, suma un error al actor y descarta cada bloqueante. El dominio declara los efectos de error y de flip/unflip con fase, lock y deadline esperados; `GameUseCases.playCard` guarda y publica esos hechos sin recalcularlos y `EffectUseCases` retorna cada callback al dominio. Un callback vencido o stale no muta. Tras el overlay, el hecho de continuidad expresa pausa, nivel completo o derrota sin que transporte recalcule bloqueantes u outcome; la CPU se programa solo tras una continuidad aceptada y `domain/cards.ts` vuelve a identificar su carta al vencer. La pausa automatica tras error no representa una solicitud de jugador y no emite `game:paused` ni su mensaje/log de pausa solicitada.

Invariantes confirmadas de estrella: el proponente se autoacepta y CPU conectada se autoacepta; el consenso exige a toda persona conectada, incluso sin cartas, pero el preview y settlement incluyen cada mano no vacia, incluso desconectada. Al consenso se consume una estrella exactamente una vez, se guarda un preview sin mutar manos y se declara un lock `star` con deadline. El settlement solo acepta las expectativas originales de fase, razon y deadline, elimina exactamente las cartas del preview una vez y declara pausa o cierre de ronda sin que el shell recalcule la regla. La pausa automatica tras settlement tampoco emite `game:paused`; ese evento y el mensaje de pausa solicitada quedan reservados a `game:pause-request`. Los acks, sockets, desconexion y timeout visual no pertenecen al dominio.

Invariantes confirmadas de progresion y final: al completar un nivel se consulta una sola recompensa y se aplica una vez, con vidas limitadas a cinco y estrellas a tres. La maquina autoriza el cierre, el avance, la liberacion de readiness y las terminales; `EffectUseCases` reinyecta `next-level-expired` y `level-ready-expired` con el efecto original, por lo que un callback stale no reparte ni inicia otra ronda. El siguiente nivel usa el reparto de setup y mantiene el lock observable `level-complete`. Derrota y victoria calculan una vez el ranking sobre el `pileHistory` vigente, con `completedAt` inyectado; el ranking ordena por puntuacion descendente, desviacion ascendente y nombre.
...
## Mapa de casos de uso, permisos y poblaciones

| Familia | Caso de uso responsable | Permiso y población | Operación de dominio |
| --- | --- | --- | --- |
| Sala | `RoomUseCases.createRoom/joinRoom/leaveRoom/kickPlayer/resyncRoom` | identidad válida; kick solo host en lobby | `domain/room.ts` |
| Reconexión y desconexión | `RoomUseCases.reconnectPlayer/disconnectPlayer` | identidad existente; solo sesión activa cambia presencia | `domain/room.ts` y presencia de aplicación |
| Inicio y retry | `GameUseCases.startGame/retryGame` | host; lobby con dos conectados o terminal | `domain/setup.ts` |
| Ready y pausa | `GameUseCases.setPlayerReady/requestPause` | conectados con mano; mano vacía fuera del quorum | `domain/round.ts` |
| Jugar y CPU | `GameUseCases.playCard`; `EffectUseCases` | actor conectado con mano; CPU elegible por dominio | `domain/cards.ts` |
| Estrella | `StarUseCases.proposeStar/acceptStar/cancelStar/rejectStar/completeStarAnimation` | consenso: todos los conectados, incluso sin mano | `domain/star.ts` |
| Efectos y progreso | `EffectUseCases.materialize` | versión, fase, lock y deadline originales | ronda, cartas, progresión y estrella |

Settlement incluye toda mano no vacía, incluso desconectada. Solo humanos conectados con carta esperan visualmente; ack, desconexión activa y deadline convergen en el mismo `star-settled`, que consume el preview una vez.
